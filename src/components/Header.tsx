...
export default function Header() {
  ...
  useEffect(() => {
    try {
      const savedCep = localStorage.getItem("pneustore_cep");
      const savedDisplay = localStorage.getItem("pneustore_logradouro");
      const savedDisplay2 = localStorage.getItem("pneustore_address_display");
      if (savedCep) setCepValue(savedCep);
      if (savedDisplay) setCepDisplay(savedDisplay);
      else if (savedDisplay2) setCepDisplay(savedDisplay2);
      else if (savedCep) setCepDisplay(savedCep);
    } catch {}
  }, []);

  const handleCepConfirm = (cep: string, display: string, data: any) => {
    try {
      if (cep) localStorage.setItem("pneustore_cep", cep);
      localStorage.setItem("pneustore_logradouro", display);
      localStorage.setItem("pneustore_address_display", display);
      localStorage.setItem("pneustore_address", JSON.stringify(data));
      if (data?.logradouro) localStorage.setItem("pneustore_street", data.logradouro);
    } catch {}
    setCepValue(cep || display);
    setCepDisplay(display);
  };

  const barText = cepDisplay ? cepDisplay : "Insira seu CEP";
  // truncate visual is handled via CSS truncate, but we also keep title
  const barTitle = cepDisplay || "Insira seu CEP";
          ...
          <div className="hidden desktop:flex flex-col">
            <div className="flex justify-between items-center w-full !px-[20px] !py-[8px] h-[30px] text-[var(--color-primary)] bg-neutralBgLayout whitespace-nowrap text-xs">
              <div className="flex h-full gap-[7px]">
                <a
                  href="https://empresas.speedmax.com.br/"
                  target="_blank"
                ...
                <a
                  href="https://blog.pneustore.com.br/"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center h-full px-[12px] cursor-pointer transition-colors duration-150 hover:underline"
                >
                  ...
                  <div className="flex h-[48px] w-full max-w-[1240px] items-center justify-between !px-[50px] gap-[32px]">
                    ...
                    <div className="relative">
                      <button
                        ...
                        aria-expanded={isCepModalOpen}
                        className="btn btn-primary flex items-center justify-center text-white gap-[8px] bg-primaryPurpleDarkest !px-[15px] h-[32px] rounded-[8px] text-[14px] cursor-pointer whitespace-nowrap hover:bg-[#3a006e] transition-colors max-w-[220px]"
                      >
                        <svg className="shrink-0" width="16" height="16" fill="none" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 18 18">
                          <path
                        ...
                        <span className="truncate max-w-[150px]" title={barTitle}>
                          {barText}
                        </span>
                      </button>
                    </div>
          ...
          <div className="flex desktop:hidden flex-col !p-[16px] !gap-[16px]">
            ...
            <div className="flex flex-col bg-primaryPurpleBase text-white">
              ...
              >
                ...
                <span className="flex-1 min-w-0 truncate" title={barTitle}>
                  {barText}
                </span>
              </button>
            </div>
